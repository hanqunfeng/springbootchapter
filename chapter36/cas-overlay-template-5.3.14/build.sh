...
function gencert() {
  ...
  # 导入证书到jre
	sudo keytool -import -alias cas -file "./etc/cas/cas.cer" -keystore "$JAVA_HOME/jre/lib/security/cacerts" -storepass changeit

	# 查看证书
  #	keytool -list -v -keystore  ./etc/cas/cas.keystore -storepass changeit
  keytool -list -alias cas -keystore "$JAVA_HOME/jre/lib/security/cacerts" -storepass changeit -v
