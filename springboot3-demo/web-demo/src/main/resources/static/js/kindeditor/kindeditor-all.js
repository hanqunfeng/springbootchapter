    ...
    _extend(KUploadButton, {
        ...
        submit: function () {
            ...
            iframe.bind('load', function () {
                ...
                //json formatter 插件会导致返回结果被格式化为html，后面在解析json的时候就会报错，所以这里要关闭json插件
                // alert(str)
...
KindEditor.lang({
    ...
    uploadError: '上传错误',
    deleteImageConfirm: '确认删除该图片吗？',
    deleteDirConfirm: '确认删除该目录吗？',
    deleteDir: '删除目录',
...
KindEditor.plugin('filemanager', function (K) {
    var self = this, name = 'filemanager',
        ...
        fileManagerDeleteJson = K.undef(self.fileManagerDeleteJson, self.basePath + 'php/file_manager_delete_json.php'),
    ...
    self.plugin.filemanagerDialog = function (options) {
        ...
        function createList(result) {
            ...
            for (var i = 0, len = fileList.length; i < len; i++) {
                ...
                var confirmContent = "";
                if (data.is_photo) {
                    buttonContent = self.lang("deleteImage");
                    confirmContent = self.lang("deleteImageConfirm");
                } else if (data.is_dir) {
                    buttonContent = self.lang("deleteDir");
                    confirmContent = self.lang("deleteDirConfirm");
                }

                K(row[0].insertCell(0)).addClass('ke-cell').html("<span title='" + data.filename + "' class='btn btn-danger btn-minier icon-delete' style='margin-right: 10px'>" + buttonContent + "</span>");

                var iconUrl = imgPath + (data.is_dir ? 'folder-16.gif' : 'file-16.gif'),
                    img = K('<img src="' + iconUrl + '" width="16" height="16" alt="' + data.filename + '" align="absmiddle" />'),
                    cell0 = K(row[0].insertCell(1)).addClass('ke-cell ke-name').append(img).append(document.createTextNode(' ' + data.filename));
                ...
                $(row).find(".icon-delete").click(function (event) {
                    event.stopPropagation(); //阻止事件向上传播
                    if (!confirm(confirmContent)) return false;
                    var url = result.current_url + $(this).attr("title");
                    // alert("url=" + url);
                    //发送删除图片请求
                    var dirPath = encodeURIComponent(result.current_dir_path);
                    // alert("dirPath"+dirPath);
                    $.post(fileManagerDeleteJson, {url: url}, function (json) {
                        //重新加载目录下图片、目录信息
                        reloadPage(dirPath, orderTypeBox.val(), createList);
                        alert(json.msg);
                    }, "json");
                });
            }
        ...
        function createView(result) {
            ...
            for (var i = 0, len = fileList.length; i < len; i++) {
                ...
                // alert("moveup_dir_path=" + result.moveup_dir_path)
                // alert("moveup_dir_path=" + result.current_dir_path)
                // alert("current_url=" + result.current_url)
                ...
                var bgHtml = [
                    ...
                    // '		<div class="left"><span class="btn btn-info btn-minier icon-add">Add</span></div>',
                    '		<div style="margin-top: 10px"><span class="btn btn-danger btn-minier icon-delete">' + buttonContent + '</span></div>',
                    '	</div>',
                    '</div>'
                ].join(" ");
                ...
                $(photoDiv).find(".icon-delete").click(function (event) {
                    ...
                    var url = "";
                    if (data.is_photo) {
                        url = $(this).parents(".ke-photo").find("img:first").attr("src");
                    } else if (data.is_dir) {
                        url = result.current_url + $(this).parents(".ke-photo").find("img:first").attr("alt");
                    }
                    //发送删除图片请求
                    var dirPath = encodeURIComponent(result.current_dir_path);
                    // alert("dirPath"+dirPath);
...
KindEditor.plugin('multiimage', function (K) {
    ...
    editor.clickToolbar(name, function () {
        ...
        var html = '<div style="padding:20px;"><div class="swfupload"><div class="ke-swfupload">' +
            '<div class="ke-swfupload-top">' +
            '<div class="ke-inline-block ke-swfupload-button">' +
            '<a href="javascript:;" style="display:none" class="ke-selectPic">选择图片</a>' +
            '<span class="ke-none">' +
            '<input type="file" multiple="multiple" id="ke-filePic" class="ke-filePic" />' +
            '</span>' +
            '</div>' +
            '<div class="ke-inline-block ke-swfupload-desc">允许用户同时上传20张图片，单张图片容量不超过5MB</div>' +
            '<span class="ke-button-common ke-button-outer ke-swfupload-startupload">' +
            '<input type="button" class="ke-button-common ke-button" value="开始上传">' +
            '</span>' +
            '</div>' +
            '<div class="ke-swfupload-body"></div>' +
            '</div></div></div>';
        ...
        inputBox.addEventListener("change", function (e) {
            $.each(inputBox.files, function (i, item) {
                //添加预览,#可以给个默认图
                var pic = $('<div class="ke-piclist" url="">' +
                    '<span class="ke-pictu">' +
                    '<span class="waitUpload left" style="color: blue">等待上传...</span>' +
                    '<img src="#" style="width:10%;height:10%;margin-left: 10px" />' +
                    '<span class="btn btn-danger btn-xs ke-picdel pic_' + xindex + '" data-id="' + xindex + '" title="删除" style="corsor:pointer;margin-left: 30px">删除</span>' +
                    '<span style="margin-left: 30px">' + item.name + '</span>' +
                    '<hr></div>');
                $(".ke-swfupload-body").append(pic);
                picArr.push('pic_' + xindex);
                //添加到存储
            ...
            if (xindex > 20) {
                ...
                for (var pair of formData.entries()) {
                    ...
                    $.ajax({
                        ...
                        processData: false,
                        contentType: false,
                        async: false,//是否同步，这里必须设置为同步操作，否则下面的验证是否上传成功的逻辑会有问题
                        success: function (data) {
                            ...
                            // alert(messageCon);
                            var _parent = $('.ke-piclist');
                            for (var i = 0; i <= _parent.length; i++) {
                                if ($(_parent[i]).attr("url") == "") {
                                    $(_parent[i]).attr("url", data.url);

                                    // $(_parent[i]).find(".waitUpload").remove();
                                    $(_parent[i]).find(".waitUpload").text(messageCon);
                                    if (data.error == 0) {
                                        $(_parent[i]).find(".btn-danger").remove();
                                    }
                                    break;
                                }
                            }
        ...
        function callAll() {
            ...
            $(".ke-piclist").each(function (i, item) {
                ...
                if (_url && _url != '' && typeof (_url) != "undefined") {
                    editor.insertHtml('<img src="' + _url + '" alt="" /><br/>');
                    len++;
                }
            });
            //关闭
            if (len > 0) {
                // console.log("关闭  len:", len);
